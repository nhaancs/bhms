...
func Routes(version string) ([]Route, error) {
	...
	fileName := fmt.Sprintf("app/services/api/handlers/%s/%s.go", version, version)
	...
	f := func(n ast.Node) bool {
		...
		for _, stmt := range funcDecl.Body.List {
			...
			routes = append(routes, Route{
				...
				File:     fmt.Sprintf("app/services/api/handlers/%s/%s/%s.go", version, group, group),
			})
		}
...
func findAppFunctionFromModel(group string, funcName string) (string, bool, error) {
	...
	file, err := parser.ParseFile(fset, "app/services/api/handlers/v1/"+group+"/model.go", nil, parser.ParseComments)
	if err != nil {
...
func findFilters(group string) ([]string, error) {
	...
	file, err := parser.ParseFile(fset, "app/services/api/handlers/v1/"+group+"/filter.go", nil, parser.ParseComments)
	if err != nil {
		return nil, fmt.Errorf("ParseFile: %w", err)
	}
...
func findOrders(group string) ([]string, error) {
	...
	file, err := parser.ParseFile(fset, "app/services/api/handlers/v1/"+group+"/order.go", nil, parser.ParseComments)
	if err != nil {
		return nil, fmt.Errorf("ParseFile: %w", err)
	}
